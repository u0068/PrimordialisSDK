...
struct HookChain : HookChainBase
{
    ...
    Ret Invoke(Args... args)
    {
        for (auto& hook : hooks)
        {
            printf("Invoking hook %p\n", hook);
            hook(args...);
        }
        printf("Invoking original %p\n", original);
...
Chain* GetOrCreateChain(const char* name)
{
    ...
    if(it != chains.end())
    {
        printf("Returning existing chain.\n");
        return dynamic_cast<Chain*>(it->second);
    }

    printf("Existing chain not found, creating new chain.\n");
