...
inline void print_material_properties() {
    ...
    for (int i = 1; i < P::n_materials; i++) {
        ...
        outFile << std::format("    show_neighbor_direction: {}\n", (bool) material.show_neighbor_direction);
        ...
        outFile << std::format("    sync_health: {}\n", (bool) material.sync_health);
        outFile << std::format("    is_stem: {}\n", (bool) material.is_stem);
        ...
        outFile << std::format("    uv: (({:.0f}, {:.0f}) + 0.5) / 32\n", (material.uv.x)*32.0f-0.5f, material.uv.y*32.0f-0.5f);
        outFile << "Functions:\n";
